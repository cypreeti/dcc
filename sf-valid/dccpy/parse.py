...
def refmac_log(logfile, dic):
    ...
    '''
    if not util.check_file(100, logfile) : return
    fp=open(logfile, "r")
    dic['prog']='REFMAC'
...
def data_from_refmac_cif(inpfile, dd):
    if not os.path.exists(inpfile) or os.path.getsize(inpfile)<100: return
...
def get_dcc_avg(fr, id):
    ...
    if id==1:
        ...
        for i in range(n):
            ...
            ch=list_in[i][18:19]
            if not ch.strip() : ch='?'
            if util.is_number(list_in[i][19:23]): nr=int(list_in[i][19:23])
            ns=34
            if 'segid=' in list_in[i]: ns=47

            if util.is_number(list_in[i][ns:ns+5]): occ=occ+float(list_in[i][ns:ns+5])
            if util.is_number(list_in[i][ns+5:ns+12]): b=b+float(list_in[i][ns+5:ns+12])
            if util.is_number(list_in[i][ns+12:ns+20]): cc=cc+float(list_in[i][ns+12:ns+20])


            if i== n-1 or list_in[i][19:23] != list_in[i+1][19:23]:
                d=[nr, ch, res, old_div(cc,m), old_div(b,m), old_div(occ,m)]
                tmp.append(d)
                b, cc, occ, m=0,0,0,0
