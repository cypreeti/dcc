...
def parse_tls_range_phenix(tlsn_in, ng, chain, chain_range, fp):
    ...
    for i, x in enumerate(tmp):
        ...
        if i>0 and 'resid' in tmp[i-1]: #check inserted code & rid of char
            t=[y for y in x if y.isdigit() or y==':' or y=='-']
        ...
        if 'null' in x :
            ...
            if i>nn-2 :
                ...
                if len(tls[i+1])>2 or len(tls[i+1])<1:
                    ...
                    if (chid not in list(chain_range.keys())):
    ...
    for i, x in enumerate(tls):
        if 'all' in x:
            for y in list(chain_range.keys()):
...
#            elif (pattern(i, tls, 'chain', '?') and (nn==2 or nn==4)) : #one chain
                ...
                ch2=tls[i+4]
                if (ch2 not in list(chain_range.keys())): continue
            ...
            chid=tls[i+4]
            if (chid not in list(chain_range.keys())): continue
            ...
            if len(list(chain_range.keys()))>1:
...
def parse_tls_range_buster_1(x, chain_range):
    ...
    if ch not in list(chain_range.keys()) :
...
def calc_tlsxc(nxyz, id):
    ...
    return [old_div(xt,occmass), old_div(yt,occmass), old_div(zt,occmass)]
...
def mass_of_atom(atom):
    ...
    if atom.upper() in list(atom_mass.keys()):
    ...
    for line in fr:
        if('REMARK   3   PROGRAM     :' in line and 'REFMAC' in line):
            ...
            if (' NULL ' not in line and len(tmp) ==4 and tmp[0] == tmp[2]) :
                ...
                if tmp[0] not in list(range1.keys()) and tmp[0] != ' ' : range1[tmp[0]]=[]
...
def correct_default(line1, line, chain_range, pdbfile):
    for x,y in list(chain_range.items()):
...
def correct_range(pdbfile, res1, res2, ln, ch, res_range):
    ...
    elif (ch not in list(chain_range.keys()) and ch not in list(chain_range1.keys()) and
          ch not in list(chain_range2.keys())) :
        print(pdbfile + '; Error: Chain (%s) not in coordinate.' %ch)
        return ln
    
    n1, n2 = int(res1), int(res2) #tls residue number
    range1=[]
    if ch in list(chain_range.keys()) :
        range1.append([chain_range[ch][0], chain_range[ch][1]])
    if ch in list(chain_range1.keys()):
        range1.append([chain_range1[ch][0], chain_range1[ch][1]])
    if ch in list(chain_range2.keys()):
    ...
    if (ch in list(chain_range.keys()) and
    ...
    elif ch in list(chain_range1.keys()) and (n1 in chain1[ch] or n2 in chain1[ch]): #ligands
        line=check_res_range(pdbfile, ln,chain1, ch, n1, n2, 1)
        
    elif ch in list(chain_range2.keys()) and (n1 in chain2[ch] or n2 in chain2[ch]): #waters
        line=check_res_range(pdbfile, ln,chain2, ch, n1, n2, 2)
        
    elif ch in list(chain_range.keys()) and ((n1<0 and n2<0) or (n1 <1 and  n2 >999))  : # default (all in ch)
        poly1, poly2 = chain_range[ch][0], chain_range[ch][1]
        tmp1, tmp2, tmp3 = '','',''
        if ch in list(chain_range.keys()): #poly
            n1, n2, idd = chain_range[ch][0], chain_range[ch][1], 1
            tmp1=make_new_line(pdbfile, ln, ch, n1, n2, 1)
            
        if ch in list(chain_range1.keys()): #lig
        ...
        if ch in list(chain_range2.keys()): #water
