...
def check_ncs_cif(file):
    ...
    if '?' in  res_rms : res_rms=cif.parse_values(items,values,'_refine_ls_restr_ncs.pdbx_rms')
    ...
    if not (ens_id) : util.perror('Warning: No cif table (struct_ncs_ens) for NCS.')
    ...
    def tmp1(list1, list2, s2):   
        for n in list1:
            if n not in list2: util.perror('Error: NCS ID (%s) not in table (%s).' %(n,s2))
...
def check_res(res_ens, chain, res_ref, res_asy, res_type, res_num, res_rms):
    ...
    for i, x in enumerate(res_ens):
        ...
        if (res_type and ('POSITIONAL' not in res_type[i].upper() and
                          'TORSIONAL' not in res_type[i].upper() and
                          'LOCAL' not in res_type[i].upper() and
                          'INTERATOMIC DISTANCE' not in res_type[i].upper() and
                          'THERMAL' not in res_type[i].upper())) :
            util.perror('Warning: wrong restrain type (%s). look at row %2d in %s.' %(res_type[i], i+1, tab) )

        if  res_rms :
            rms=res_rms[i]
            if util.is_number(rms):
                if ((res_type and 'POSITIONAL' in res_type[i] and float(rms)>2) or
                    (res_type and 'TORTIONAL'  in res_type[i] and float(rms)>15 )) :
