...
import util, config, parse, cns_inp 
...
def get_resid_percent_asa(outf, id):
    ...
    if not util.check_file(10, outf) : return asa_perc

    fp=open(outf, 'r')
    for x in fp:
        if 'ATOM' not in x[:4] and 'HETATM' not in x[:6] : continue
        comp, ch, nres=x[17:20].strip(), x[20:22].strip(),x[22:26].strip()
        key='%s_%s_%s' %(comp, ch, nres)
        if key not in resid: resid[key]=[]
        if key in resid:
            if key not in keys: keys.append(key)
            resid[key].append(float(x[60:67]))

    for key in keys:
        val=old_div(sum(resid[key]),len(resid[key]))
...
def run_sfcheck(pdbfile_in, sffile, dic, detail):
    ...
    pdbfile=pdbfile_in
    if idd: pdbfile= cif.cif2pdb(pdbfile_in)
    ...
    if idd : util.delete_file(pdbfile)
    return dic1, cif_new
...
3. parse cc.log
    ...
    if type1 == 'xtriage' :
        ...
        if vers >= 1013:
            ...
            resol=' '        
            if '?' not in dic['resh'] and 'n' not in dic['exp']: resol='high_resolution=%s' %dic['resh']

            lib=' '
            if dic['lib'] : lib=dic['lib']
            maps=''
            if dic['rsrd'] : maps= ' map="2mFo-DFc" '
            ...
            if not util.check_file(30,outfile) :
                ...
                os.system(arg)
                if (dic['verb']==0) : os.system('rm -f elbow__LIG.*')
            ...
            if dic['rsrd'] and util.check_file(30,mtzfile) : #get density correlation
                ...
                if os.path.dirname(dir_tmp) : os.system('rm -rf %s ' %dir_tmp)
                arg='phenix.get_cc_mtz_pdb  pdb_in="%s"  mtz_in="%s" \
                labin="FP=2mFobs-DFmodel PHIB=P2mFobs-DFmodel" resolution=%s \
                temp_dir = "%s" > %s ' %(pdbfile,mtzfile,dic['resh'],dir_tmp,cc_log)
    ...
    if not dic['verb'] : util.delete_file(mtz)
    return outfile
...
def get_matthew_coeff(pdbfile):
    ...
    if not util.check_file(30, outf): return  matth_coeff, solvent
...
#    os.system(arg)
    if(dic['verb']==0): util.delete_file(mtz3)
...
def do_extra_check(cns_pdb):
    ...
    if not util.check_file(10, cns_pdb): return [], ''
    fp=open(cns_pdb, 'r')
    lig=[]
    for x in fp:
        res=x[17:20]
        if res not in util.residue() and '9999.0009999.0' in x:
            if res not in lig : lig.append(res)
...
def run_cns(dic, pdb_new, cns_sf):
    ...
    pdb_tmp=pdb_new
    if util.is_cif(pdb_new): pdb_tmp=cif.cif2pdb(pdb_new)
...
#    pdb_1 = fix_dna_rna_cns(pdb_tmp)
    ...
    if(dic['verb']==0): 
        ...
        if dic['cif']: util.delete_file(pdb_tmp)
...
def cns_param_top(pdb_new):
    ...
    if not lig : return param, top

    fw1=open(param,'w')
    fw2=open(top,'w')
    print(('list of unique ligands:', list(lig.keys()))) 
    for x in list(lig.keys()):
        ligpdb='%s.pdb' %x.strip()
        fw=open(ligpdb, 'w')
        for y in lig[x]: fw.write(y)
        fw.close()
        run_xplo2d(ligpdb)
        ...
        if util.check_file(10, '%s.par' %ligname):
            for y in open(parfile, 'r').readlines() : fw1.write(y)
            
        if util.check_file(10, '%s.top' %ligname):
            for y in open(topfile, 'r').readlines() : fw2.write(y)
...
''' %(maptype,f1,phi,  mtz, size, f1,phi,  pdbfile,mapout)
...
#    print('The generated map = %s' %mapout)
...
quit
EOF
    ''' %(mapman, map1, map2, pdbfile, edsout, radius)
...
cell 1
eof
    ''' %(xyzfile,ncont_log, chid, nseq, radius )
...
def run_eds_fem(mtzfile, pdbfile, dic, dic3):
    ...
    if dic['xyzlim']: #do mapmask by xylim in fract coord
        ...
        resh=2.0
        if util.is_number(dic['resh']): resh = float(dic['resh'])
        script = 'mtz2eds_xyzlim.csh'
        xyzlim=find_xyzlimt(4.1, pdbfile) #extend 4.1
        gen_mtz2eds_script_xyzlim(xyzlim, resh, script)
    else:
        if dic['map_dsn6']: arg  = arg + ' -map_dsn6 '
        if dic['map']  : arg  = arg + ' -map '
        if dic['noeds'] : arg  = arg + ' -noeds '
        script = 'mtz2eds.csh'
#        gen_mtz2eds_script(script,'FEM', 'PHIFEM') #fem
    ...
    dic3['rsrd'] = dic['rsrd']
    cifrefm1=write_cif.eds2cif(edsout, dic3) # get edsout in cif
    if not dic['verb'] : util.delete_file('lx_mapman.log' , 'mtz2eds_xyzlim.csh')
...
def run_ed(mtzfile, pdbfile, dic, dic3):
    ...
    if not util.check_file(100, pdbfile) : return ''
    
    out1=pdbfile + '_edstat.out'
    log1=pdbfile + '_edstat.log'
    dic3['pdb_new']=pdbfile
    eddir=''
    if dic['eddir'] != '?' : eddir=' -tmpdir %s ' %dic['eddir'] 
    ...
    if not dic['verb']: util.delete_file(out1, log1)
...
def split_pdb_edstat(pdb):
    ...
    na=0
    for ch in list(record.keys()):
        nres=list(record[ch].keys()) 
        etype=type_of_chain(nres, record[ch])
        if etype == 'na' :
            na=1
            break

    if na==0: return '', '', '' #no NA involved! 
...
def getpdb_na(fp, id):
    ...
    for x in fp:
        ...
        #if id=='sugar':
            if not (" C1'" in atom or " C2'" in atom or " C3'" in atom or
                " O4'" in atom or " C4'" in atom or " C5'" in atom or
                " O2'" in atom  ): #sugar
        ...
        #elif id=='phos' :
            if not (' P  ' in atom or ' OP1' in atom or ' OP2' in atom or " O3'" in atom or
                  ' OP3' in atom or " O5'" in atom  ): #phosphate
                t=''.join([x[:54] , '  0.00', x[60:]])
                fw.write(t)
            else:
                fw.write(x)
        ...
        #elif id=='side' :
            if (' P  ' in atom or ' OP1' in atom or ' OP2' in atom or " O3'" in atom or
                  ' OP3' in atom or " O5'" in atom or
                " C1'" in atom or " C2'" in atom or " C3'" in atom or
                " O4'" in atom or " C4'" in atom or " C5'" in atom or
                " O2'" in atom 
               ): #phosphate
                t=''.join([x[:54] , '  0.00', x[60:]])
...
def split_pdb(pdb):
    ...
    for ch in list(record.keys()):
        ...
        nres=sorted(record[ch].keys())  #newer version of python
        etype=type_of_chain(nres, record[ch])
        phos=[]   
        nr_len=len(nres)
        for i, n in enumerate(nres) :
            atoms=record[ch][n]
            comp=atoms[0][17:20]
            if 'HOH' in comp or 'DOD' in comp : continue
            if comp not in util.protein() and comp not in util.dna_rna() : continue
            for x in atoms:
                if 'ATOM' not in x[:4] and 'HETA' not in x[:4]: continue
                atom=x[12:16]
                if etype == 'prot' :
                    if (' N  ' in atom or ' CA ' in atom or ' C  ' in atom or
                        ' O  ' in atom or ' CB ' in atom ):
                        fw1.write(x) #main chain
                    else:
                        fw2.write(x)  #sidechain

                elif etype == 'na' :
                    if (' P  ' in atom or ' OP1' in atom or ' OP2' in atom or
                        ' OP3' in atom or " O5'" in atom  ): #phosphate
                        phos.append(x)
                    ...
                    elif (" C1'" in atom or " C2'" in atom or " C3'" in atom or
                          " O4'" in atom or " C4'" in atom or " C5'" in atom or
                          " O2'" in atom  ): #sugar
                        fw1.write(x)
                    else:
        ...
        for z in phos_order : fw3.write(''.join(z))
    fw1.close(),fw2.close(), fw3.close()

    return out1,out2,out3
        
##########################################################
def type_of_chain(nres, record):
    ...
    for y in nres:
        ...
        for x in record[y]:
            ...
            if comp in util.protein() :
                ...
                etype='other'
        if etype=='prot' or etype=='na': break
    return etype
...
def run_refmac(pdbfile, sffile, dic, idd, option):
    ...
    if dic['cif']>0: #the cif version depend on current path
        if pdbfile[0] != '.' and pdbfile[0] != '/': pdbfile = './'+pdbfile
        if sffile[0] != '.' and sffile[0] != '/': sffile = './'+ sffile
            
    arg = pdbfile + ' ' +  sffile + ' -mtz ' + option

    if dic['refmac_tls'] : arg  = arg + ' -tls '
    if '-scale' not in option and dic['scale']>0 : arg = arg + ' -scale '
    if '-rest'  not in option and dic['rest']>0 :  arg = arg + ' -rest '
#    if '-reso'  not in option and dic['reso']>0 :  arg = arg + ' -reso '
    arg = arg + ' -reso '
    if '-wave'  not in option and dic['wave']>0 :  arg = arg + ' -wave '
    if '-nofree' not in option and dic['status']=='N' : arg = arg + ' -nofree '
...
def run_refmac_more(pdb, mtz, dic, dici ): 
    ...
    if dic['omitmap'] : rcut=0.03  #3.0% difference tolerance
    if (util.is_number(dic['resh']) and float(dic['resh'])<=1.5) : rcut=0.010

    dicout=main.initialize_dic1()    
    if (not (util.is_number(dic['rfact']) and util.is_number(dici['rfact'])) or
        float(dici['rfact']) - float(dic['rfact']) < rcut ): 
        return '', '','','',dicout
...
#    option=[' -twin ',  ' -scale ', ' -scale -twin ', ' -rest -wave ', ' -reso ']
    ...
    for i,x in enumerate(option):
        ...
        if (util.is_number(dicx['fom']) and float(dicx['fom']) >0.6 and 
            util.is_number(dicx['rfact']) and util.is_number(dic['rfact']) and 
            float(dicx['rfact']) - float(dic['rfact'])<rcut ):
            
            if ('-twin ' in option[i] and float(dici['rfact']) - float(dic['rfact'])>0.05 and
                '2:' in dic['twin_fr'] ):
...
""" % (xyzlim,xyzlim,xyzlim,resh,mapman)
...
############## End of script ############
""" % (config.PATH1,ampl,phase, mapman, mapman, mapman)
...
def run_pointless(mtz, dic):
    ...
    if not dic['verb'] : util.delete_file(out_pointless)
