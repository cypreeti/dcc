#!/nfs/msd/services/onedep/resources/tools/tools-redhat-7-development/bin/python
#!/usr/bin/env python
...
import config, tls, ligand, sdsc_ligmap, prog, parse,matt, util, ncs, write_cif
...
#    dic['software'].append(['DCC', config.VERSION.split()[0],config.DCC])
    ...
    if (dic['ligmap'] and not dic['ligmapcif']): 
        if ((not dic['sdsc_map'] and not check_ligand(pdbfile, 0)) or 
            (dic['sdsc_map'] and not check_ligand(pdbfile, 1)) ):
    ...
    if dic['mtzmap']: # run fft with the mtz containing phase
        ...
        if dic['display']:
            ...
            os.system(command)
        if (dic['verb']==0) : util.delete_file ('get_mtz2map.csh')
        print ("Time elapsed : %.2f(sec)" % (time() - start_time))
 
        sys.exit()

    sffile = check_sf(dic) #get new sf (also for neutron) & update dic
    if util.check_file(10, pdbfile, sffile)==0 : return
    
    if dic['omit']==1 and 'omit_residue' in dic : pdbfile=omit_residue(dic, pdbfile)
    ...
    if(dic['verb']==0): # clean files
        ...
        if dic['pdb_tls']: util.delete_file(dic['pdb_tls'])
        ...
        if (dic['rsrd']): util.delete_file('TMP_PROMOTIF_FILE.???')
        if dic['xyz_type']=='cif' : util.delete_file(dic['pdbfile_orig'])
        if dic['ligmapcif']:
            util.delete_file(dic['xyzfile_orig']+'.PDB_new', dic['xyzfile_orig']+'.PDB_new_new')
            if dic['omitmap']: util.delete_file(dic['xyzfile_orig']+'.PDB_new_NOLIG_new')
...
def get_argument(arg, dic):
    ...
    for k in range(narg):
        if(arg[k].upper() =="-PDB" or arg[k].upper() =="-CIF"):
         ...
         #generate all the ligand related stuff
            dic['ligmap']=1
            dic['refmac']=1
            if arg[k].upper() =="-LIGMAPCIF" : dic['ligmapcif']=1
            ...
            asa=prog.calc_asa_areaimol(arg[k+1], 1)
            sys.exit()
            
        elif(arg[k].upper() =="-MOTIF" ):
            asa=prog.run_motif(arg[k+1]) # a pdbfile
            sys.exit()
    ...
    if (not (dic['sfcheck'] or dic['phenix_x'] or dic['phenix_n'] or dic['refmac'] or 
        dic['phenix_xn'] or dic['cns'] or dic['shelx'] or dic['auto'] or dic['fem'])): 
...
def check_env(dic):
    ...
    if (dic['phenix_xtriage'] or dic['phenix_x'] or dic['phenix_n'] or
        dic['exp']=='n' or dic['exp']=='xn' or dic['phenix_xn'] or dic['fem']):
        if 'PHENIX' not in os.environ :
            ...
            if ss not in dic['software']: dic['software'].append(ss)
            if dic['no_xtriage']==0:
...
#        check_cif(xyzfile, dic) #check cif and update dic
    ...
    if dic['omitmap']: dic['pdbfile']=ligand.remove_ligand(dic['pdbfile'])
    pdbfile=dic['pdbfile']
    ...
    if dic['tls_attempt'] == 'Y' : #from check_pdb
        ...
        if tlsok :
            dic['tls_ok'] = 'Y'
            dic['pdb_tls'] = pdb_tls
            if dic['omit']==1 and 'omit_residue' in dic : dic['pdb_tls']=omit_residue(dic, pdb_tls)
...
def do_validation_auto(dic, pdb_new, sf_new):
    ...
    if 'n' in dic['exp'] or dic['phenix_n'] or dic['phenix_xn'] : return dd_all  #neutron diffr
    ...
    cond_cut=(util.is_number(dic['rfact']) and util.is_number(dd['rfact']) and
              (float(dd['rfact'])-float(dic['rfact'])<=rcut))
    if cond_cut : return dd_all
...
#    prog_test=['refmac', 'phenix_x', 'cns', 'sfcheck']
    ...
    dd_all_new=[]
    if '?' not in dd['rfact'] : dd_all_new.append([dd, dd_all])
    for x in prog_test:
        if x==pp: continue
        dic[x]=1            
        print('\nTesting program  %s ...' %x)
        ...
        cond_cut=(util.is_number(dic['rfact']) and util.is_number(dd_in['rfact']) and
                  (float(dd_in['rfact'])-float(dic['rfact'])<=rcut))
        if cond_cut or '?' in dic['rfact'] : 
            return dd_all_in
        elif '?' not in dd_in['rfact']:
            dd_all_new.append([dd_in, dd_all_in]) 

    if len(dd_all_new)==0 : return dd_all  # more test failed
...
########  run sfcheck
    ...
    if(dic['sfcheck']):
        ...
        if tlsok :(dic2, cifsfch2)=prog.run_sfcheck(pdb_tls,sf_new, dic,'TLS')
        nn=best_of_solutions([dic1, dic2])
        if tlsok : tell_btype(nn, dic)
        dic['cifsfch']=cifsfch1
        if nn==1: dic['cifsfch']=cifsfch2
...
######  run refmac : multiple tries (non-large file) if needed.
    ...
    if(dic['refmac']):
        ...
        edsout, edsout1,edsout2,edsout3 ='','','',''
        if dic['rsr_all'] >0: #all, main/side chain, update cifrefm in dic
            ...
            if dic['map']: 
                ...
                if not dic['verb']: util.delete_file('get_mtz2map.csh')
        else:
            if not dic['cc']: 
        ...
        for i in range(len(mtzs)):
            if i==nn and dic['verb'] : continue 
            util.delete_file(mtzs[i], pdbs[i], logs[i])
        if not dic['verb']: util.delete_file(edsout)
...
#### run ligand map cut (always position it after runing refmac!)
    ...
    if dic['ligmap'] or dic['ligmapcif']:
        ...
        if not dic['verb'] : util.delete_file(map2fofc)
...
####  run phenix (model_vs_data/xtriage)
    ...
    phlog1,phlog2,phlog3,phlog4,phlog5='','','','',''
        
    if not dic['no_xtriage'] and not dic['fem'] and 'PHENIX' in os.environ  : #always calc 
        phlog1 = prog.run_phenix(dic, dic, 'xtriage')

    if(dic['phenix_x'] ):
        phlog2 = prog.run_phenix(dic, dic6, 'xray')
        dic6['detail'] = 'Calculated Xray'
    if(dic['phenix_n']):
        phlog3 = prog.run_phenix(dic, dic7, 'neut')
        dic7['detail'] = 'Calculated Neut'
    if(dic['phenix_xn'] or dic['exp']=='xn' or dic['exp']=='nx'):
        phlog4 = prog.run_phenix(dic, dic8, 'xray')
        dic8['detail'] = 'Calculated Xray'
        phlog5 = prog.run_phenix(dic, dic9, 'neut')
...
#### run cns program (model_stat)
    ...
    if dic['cns'] and 'CNS_SOLVE'  in os.environ :
        ...
        if tlsok : tell_btype(nn, dic)
        if(dic['verb']==0): util.delete_file(' model_stats.list model_stats.log')
...
#---------  write the final data
    ...
    if '?' in dic['prog'] : dic['prog'] = 'AUTH'  #the report program
    ...
    if dic['rsrd']==0:  add_modelid_to_outfile(dic, outfile)
    if dic['lldf']: add_lldf_to_outfile(dic, outfile)
    
    newfile=cif.cif2cif(outfile) 
    shutil.move(newfile, outfile)
    print ("\nThe final mmCIF file = %s\n" % outfile)

    if dic['edstat'] >0: util.delete_file('NEW_sugar.pdb* NEW_side.pdb* NEW_phos.pdb*') 
    ...
    if(dic['verb']==0): # clean files
        ...
        if dic['omitmap'] : util.delete_file(dic['pdb_nolig'], 'EDS_OMIT.OUT.cif')
...
def best_of_solutions(dd):
    ...
    d1 = []
    for x in dd:
        if not util.is_number(x['rfact']) : continue
        d1.append(float(x['rfact']))

    if len(d1)==0 : return n
    min_r = min(d1)
    max_r = max(d1)
...
def get_segid_cif(xyzf):
    ...
    if not ins: ins=cif.parse_values(items,values,"_atom_site.ndb_ins_code");
...
#    print 'ch_lig,chr_lig=', ch_lig,chr_lig
    ...
    for x in dcc:
        ...
        if chid in list(ch_lig.keys()) and nseq in ch_lig[chid] : #found ligand
            ligid=[x[0], x[1], x[2]]
...
#            print 'all_resid=', lldf,  all_resid
    ...
    for i, x in enumerate(flist):  #writing 
        fw.write(x)
        if ('#' in x[:10] and 'Overall properties from' in flist[i+1] and
            len(all_lldf)>0) : 
...
def get_contact_resid(logfile):
    ...
    data=[]
    if not util.check_file(100, logfile) : return data
...
#    if dic['model'] != 'Y' : return
    ...
    for i, x in enumerate(fp):
        if (i>0 and (('rscc_mapman.' in fp[i-1]  and '_mapman.' not in x) or
                     ('_sfcheck.' in fp[i-1]  and '_sfcheck.' not in x))):
            firstl=x
            break
    if not  firstl : return
    ...
    for x in fp:
        ...
        if  k1==0 and ('_pdbx_rscc_mapman.' in x):
            ...
            if k1>0 :
                ...
                if id == id_mapman : nmod=nmod+1
            elif k2>0:
                id=[t[0],t[1], t[2],t[3]]
                if id == id_sfcheck : nmod=nmod+1
            else:
                nmod=1
...
def add_pdbid_to_outfile(dic, file):
    ...
    if not util.check_file(file): return
    ...
    n1, j, n, m=0,0,0,0
    beg, end= resid[pdbid[j]][0], resid[pdbid[j]][1]
    for ln in lis:
        n1=n1+1
        tmp = ln.split()
        ...
        if  n==0 and  '_pdbx_rscc_mapman.' in tmp[0]:
            ...
            if (tmp[4] == end[2].strip() and tmp[2] == end[1].strip() and
                tmp[3] == end[0].strip() ):
                j=j+1
                if j>=len(pdbid): j=len(pdbid)-1
                beg, end= resid[pdbid[j]][0], resid[pdbid[j]][1]
        elif m>10 and '#' in ln:
            break

        fo.write(s)
    for ln in lis[n1-1:]: fo.write(ln)
...
def check_sf(dic):
    ...
    if os.path.exists(sf_valid) : util.delete_file(sf_valid)
    if os.path.exists(sf_stat) : util.delete_file(sf_stat)
    ...
    n1=0
    for line in fp:
        if (' x ' in line or ' h ' in line or ' l ' in line or
            ' < ' in line or ' - ' in line ): continue
        if '_refln.pdbx_r_free_flag' in line : line ='_refln.pdbx_r_free_flagx\n'
        if '_refln.pdbx_F_calc_with_solvent' in line : line='_refln.fake2\n'
        if '_refln.pdbx_phase_calc_with_solvent' in line : line='_refln.fake3\n'
        n1=n1+1
        if n1>200 and "data_" in line: break
        fw1.write(line)
    fw1.close()
    ...
    if not dic['verb']: util.delete_file(sf_new) 
    return sf_valid
...
#    print 'Extracting SF statistics from %s' %sf_stat
    ...
    dic['status_used'] = 'N'
    if not util.check_file(100, sf_stat): return ''
    
    fp=open(sf_stat, 'r').readlines()
    for x in fp:
        if '_sf_convert.error' in x : continue
    ...
    for x in fp:
        if 'Use data with resolution' in x : break
        ...
        elif 'Number of reflections for validation' in x : break
...
def separate_sf(sffile):
    ...
    for ln in fp:
        n1=n1+1
        if n1>200 and "data_" in ln: break
        fw1.write(ln)
...
#    print 'Checking cif file=%s' %xyzfile
    ...
    if not util.check_file(100, xyzfile): return
...
###
    ...
    if len(dbid)>1 and len(dbtype)>1: 
        ...
        for i, x in enumerate(dbid):
            if dbtype[i]=='split' : dic['split2'] = dic['split2'] + dbid[i] + ' ' 

###
    items,values = cif.cifparse(flist, '_atom_site.')
    model=cif.parse_values(items,values, '_atom_site.pdbx_PDB_model_num')
    if not model : model=cif.parse_values(items,values, '_atom_site.ndb_model')
    if model:
        if int(model[-1])>1 : dic['model']='Y'
...
def check_pdb(pdbfile, dic):
    ...
    if not util.check_file(50, pdbfile): return 'XXXX'
    ...
    prob_b, prob_occ=0,0
    biso_r, bmat, nmat, xatom, natom, nanis = '','',0,0,0,0
    ...
    for line in fr:
        ...
        if line[0:6] == 'HEADER' :
            ...
            if not len(dic['pdbid']) or len(dic['pdbid'])!=4 : dic['pdbid']='xxxx'
            ...
            if line[26:].strip(): dic['prog'] = line[26:].split()[0].upper()
            
        elif "REMARK   3   RESOLUTION RANGE HIGH (ANGSTROMS) :" in line:
            if not neut : dic['resh'] = util.get_value_after_id(line,":")
            if neut : dic['resh_n'] = util.get_value_after_id(line,":")
            
        elif "REMARK   3   RESOLUTION RANGE LOW  (ANGSTROMS) :" in line:
            if not neut : dic['resl'] = util.get_value_after_id(line,":")
            if neut : dic['resl_n'] = util.get_value_after_id(line,":")
            
        elif "REMARK   3   COMPLETENESS " in line:
           
            if not neut : dic['comp'] = util.get_value_after_id(line,":")
            if neut : dic['comp_n'] = util.get_value_after_id(line,":")
            
        elif ("REMARK   3   NUMBER OF REFLECTIONS             " in line or
              'NUMBER OF REFLECTIONS   (NO CUTOFF)' in line):
            if not neut :dic['nref'] =  util.get_value_after_id(line,":")
            if neut : dic['nref_n'] =  util.get_value_after_id(line,":")
            
        elif (" MEAN B VALUE      (OVERALL, A**2) : " in line ):
            biso_r=util.get_value_after_id(line,":")


        elif (('REMARK   3   R VALUE' in line[:21] and '(WORKING + TEST SET)' in line[22:47]) or
              ('R VALUE   (WORKING + TEST SET, NO CUTOFF) :' in line)):
            if not neut : dic['rall'] = util.get_value_after_id(line,':')
            if neut : dic['rall_n'] = util.get_value_after_id(line,':')
                       
        elif (('REMARK   3   R VALUE' in line[:21] and ' (WORKING SET)' in line[:47]) or
              ('R VALUE          (WORKING SET, NO CUTOFF) : ' in line)):
            if not neut :dic['rfact'] =  util.get_value_after_id(line,':')
            if neut :dic['rfact_n'] =  util.get_value_after_id(line,':')

        elif ('REMARK   3   FREE R VALUE                    ' in line or
              'FREE R VALUE                  (NO CUTOFF' in line ):
            if not neut :dic['rfree'] =  util.get_value_after_id(line,':')
            if neut :dic['rfree_n'] =  util.get_value_after_id(line,':')

        elif 'REMARK   3   FREE R VALUE TEST SET SIZE  ' in line:
            if not neut :dic['pfree'] =  util.get_value_after_id(line,':')
            if neut :dic['pfree_n'] =  util.get_value_after_id(line,':')

        elif ('REMARK   3   FREE R VALUE TEST SET COUNT    ' in line or
              'FREE R VALUE TEST SET COUNT   (NO CUTOFF)' in line ):
            if not neut : dic['nfree'] =  util.get_value_after_id(line,':')
            if neut : dic['nfree_n'] =  util.get_value_after_id(line,':')
           ...
           util.perror('Warning: PDB header shows B factor "RESIDUAL ONLY". Check needed!')  
            ...
            if ncs.isdigit() and int(ncs)>0 : dic['ncs'] = int(ncs)

        elif ('REMARK   3' in line and
              ('  FRACTION:' in line or 'TWIN FRACTION :' in line )) :
            fract=line.split(':')[1]
            if util.is_number(fract) and float(fract)>0 : dic['twin'] = 'Y'
            ...
            if(len(cell)>30 and len(spg)>1) : check_cell_spg(cell, spg)
...
#                util.perror('Error in pdb! %s' %line)
                ...
                natom=natom+1  
                if (len(m285) ==3 and len(bmat)==0 ): line = new_xyz(m285, line)
            ...
            elif (' D'  in line[76:78] and 'exp' in dic and
                  ((dic['exp'] =='x' or dic['exp'] =='e') and dic['phenix_n']==0 and dic['phenix_xn']==0 )) :
        ...
        fw.write(line)
    if(prob_b>0): util.perror('Warning: Number of Bfactor problems =%d' %prob_b)
    if(prob_occ>0): util.perror('Warning: Number of Occupancy problems =%d' %prob_occ)
    
    if dic['model'] == 'Y':
        print ("Note: The PDB file %s has multiple models." % pdbfile)


    if xatom>0 : util.perror('Warning: %d X atoms were assigned to C atom for validation!' %xatom)
    ...
    if dic['rfact']=='?' and dic['rall']!='?' : dic['rfact']=dic['rall']
...
#    matt_coeff ,solvent = prog.get_matthew_coeff(pdbfile)  #using a CCP4 module
    ...
    if len(biso)>0:
        ...
        b=round(old_div(sum(biso),len(biso)),3)
        dic['biso']= str(b)
        if util.is_number(biso_r):
            if abs(float(biso_r) - b)>10 or old_div(abs(float(biso_r) - b),b) >0.10 :
...
def coord_by_biomt(pdbfile, m285, btrix, bmat):
    ...
    for x in fp:
        if (x[:4] == 'ATOM' or x[:6] == 'HETATM'):
             atom.append(x)
        elif ('CONECT' not in x[:6] and 'MASTER' not in x[:6] and
              'END' not in x[:3]):
            fw.write(x)
            pdbhead.append(x)
...
def new_xyz(mt, line):
    ...
    if line[:4] != 'ATOM' and line[:6] != 'HETATM' or len(line)<54: return line
...
def initialize_dic():
    ...
    dic={'map':0, 'map_dsn6':0,'verb':0, 'refmac':0, 'refmac_tls':0, 'refmactwin':0,
         'scale':0,  'outfile':'','no_xtriage':0, 'reso':0,'sfcheck':0, 'wave':0,
         'phenix_x':0, 'phenix_n':0, 'phenix_xn':0, 'phenix_xtriage':0,'refine':0,
         'cns':0,  'ligmap':0, 'sdsc_map':0,'ligmapcif':0, 'shelx':0, 'display':0,
         ...
         'tls_ok':'N', 'status':'N', 'status_used':'N', 'isob':'FULL', 'mtr':0,  
         'exp':'x', 'error':'', 'twin_op':'', 'twin_fr':'','ncs':0,'tls':0,
         'xyzlim':0,'noeds':0, 'omitmap':0, 'rsrd':0,'rsr_all':0,'edstat':0,'cc':0,
         'anom_f':0, 'anom_i':0,'sf_i':0,'sf_sigi':0,'sf_f':0,'sf_sigf':0,
         'fem':0,'omit':0, 'lldf':0, 'software':[],'natom':0
         }  #options
...
def update_bfull(dic):
    ...
    if not outfile :  outfile = file + '_bfull'
    ...
    if int(dic['tls'])>0 and 'Y' not in dic['anis'] : #regular update
        ...
        if not util.check_file(100, mtz) : return
...
'''
    ...
    for x in fp: #get new B
        if 'ATOM' in x[:4] or 'HETA' in x[:4] : bnew.append(x[60:66])
        if 'ANISOU' in x[:6]:
    ...
    if idd==0: #add aniso
        ...
        for x in new_flist: fw.write(x)

        fw.write(anis)
        nlen=len(u11)
        for i in range(nlen):
            
            if not symbol[i] : symbol[i]='.'
            if not alt[i]: alt[i]='.'
            if not chain[i]: chain[i]='.'
            if not ins[i]: ins[i]='.'
...
#        print nu11,nu22,nu33,nu12,nu13,nu23
        ...
        newlist.append('#\nloop_\n')
        for x in items: newlist.append('%s \n'%x)
        for x in new_rows:
            s1=''.join(x) +'\n'
            newlist.append(s1)
...
def update_uij(flist,bnew):
    ...
    if b in items : nb=items.index(b)
...
def check_result(d1, d2):
    ...
    if (util.is_number(d1['resh']) and float(d1['resh'])>1.7 and
        d1['tls_report']=='N' and d1['anis']=='Y' and d1['model']=='?'):
        info.append('Warning: ANISOU records exists but no TLS records (resolution=%sA).' %d1['resh'])
        
    if (util.is_number(d1['resh']) and float(d1['resh'])<1.5 and d1['tls_report']=='Y' and
        util.is_number(d1['rfact']) and util.is_number(d2['rfact']) and
        float(d2['rfact']) - float(d1['rfact']) >0.015 and 'REFMAC' in d1['prog'].upper())   :
...
#            info.append('         Data may have problem outside of the resolution range(%s - %s).' %(d1['resl'], d1['resh']))
    ...
    if (util.is_number(d1['nfree']) and util.is_number(d2['nfree']) and
        util.is_number(d1['resh']) and util.is_number(d2['resh'])):  #Nfree
    ...
    if '?' in d2['rfact'] : info.append('Error: SF validation failed.')
...
def check_error(pdbfile, sffile, dic):
    ...
    if not util.check_file (500, pdbfile, sffile): return
    ...
    if len(spg_sf)>1 and len(spg_pdb)>1 :
        ...
        if (len(d1)==4 and d1[1] == d1[3] == '1'): spg_sf = d1[0] + d1[2]
        if (len(d2)==4 and d2[1] == d2[3] == '1'): spg_pdb= d2[0] + d2[2]
...
def remove_onecif_table(cif, table):
    ...
    for i in range(0, m):
        if table in cif[i].strip()[:len(table)+1]:
            n1=i
            if 'loop_' in cif[i-1] : n1=i-1
            for j in range(i, m):
                if ('#' in cif[j][:1] or 'loop_' in cif[j][:5] or
                    ('_' in  cif[j][:1] and table not in cif[j])) :
                    n2=j
                    break
            if n2>n1 : break
    #print(len(table), len(cif),  n1, n2)       
    if n2<len(cif) and n2>0: del(cif[n1:n2])
    return cif
            ...
            for y in v :
                ...
                for x in pdb:
                    if (('ATOM' in x[:4] or 'HETATM' in x[:6]) and
                        k==x[20:22].strip() and int(x[22:26])==y):
                        natm=natm+1
                      #  print natm, k, v, x
                        if natm>1 : return 1
    if id==0:
        for k, v in ch_pep.items():
            if len(v)<15: return 1
...
def check_bfactor(pdbfile):
    ...
    if os.path.exists(pdbfile) and os.path.getsize(pdbfile) >1000 :
        ...
        fr.close()
    n=len(negb)
    if bigb>0: util.perror('Error: There are %d atom with B_full>999.' %(bigb))
    if n>0: util.perror('Error: There are (%d) atom with B_full<0.' %(n))
...
def add_const_to_biso(pdbfile, minb, id):
    ...
    fw=open(out, 'w')
    for x in fp:
        if id==0 and 'ANISOU' in  x[:6] : continue
