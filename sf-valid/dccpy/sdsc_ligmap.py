...
def sdsc_ligand(dic, mapfile, pdbfile) :     
    ...
    if conn: 
        nn=len(conn['id'])
        for i in range(nn):
            if 'covale' not in conn['id'][i] : continue
    ...
    if nonploy:
        ...
        for i in range(nn):
            if ('MG' == nonploy['monid'][i] or 'CL' == nonploy['monid'] or
                'HOH' in nonploy['monid'][i] or 'DOD' in nonploy['monid'][i]):
                continue 
            ...
            if ch not in list(lig_ch.keys()): lig_ch[ch]=[]
            lig_ch[ch].append(res)
           
    lig_group=[] #group the connected ligands
    for ch, lig in lig_ch.items():
        tmp=get_lig_group(lig, res_pdb)
        if tmp: lig_group.extend(tmp)
    ...
    if prd:
        ...
        for i in range(nn):
            asym=prd['asm_id'][i]
            if asym not in poly['asym'] : continue
            prd_tmp=[]
            for j, x in enumerate(poly['asym']):
                if asym==x:
                    comp, ch, nres=poly['monid'][j], poly['chid'][j], poly['nseq'][j]
                    if '?' in comp: continue  #not in pdb
                    res='%s_._%s_%s'%(comp, ch, nres)
                    res_pdb[res]=residue_xyz(comp, ch, nres, pdblist)
                    prd_tmp.append(res)
            
            if prd_tmp: prd_all.append(prd_tmp)
    if prd_all: lig_group.extend(prd_all)
...
def get_lig_group(liglist, res_pdb):
    ...
    if len(liglist)==1: return [liglist]
