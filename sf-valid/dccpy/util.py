...
def chain_res_atom(pdb):
    ...
    for x in fp:
        print(x)
        if not('ATOM' in x[:4] or 'HETATM' in x[:6] or len((x.strip()))<50):continue ##x.strip() <50, assuming that it is comparing the len(x.strip())<50 and x.strip()<50 are equivalent! 
        ch=x[20:22].strip()
        if ch==' ': id=1
           
        if ch not in list(dd.keys()) and ch != ' ' : dd[ch]={}
        res=x[22:27] #include inserted
        if ch !=' ' and ch in list(dd.keys()) and res not in list(dd[ch].keys()) : dd[ch][res]=[]
...
#    pth='/net/data/remediation-alt/ftp-v4.0/pdb/data/structures/divided' #old path
...
def frac_orth_matrix(cell):
    ...
    vol=sg*sb*sa
    frac[0][0]=1.0/a
    frac[0][1]=old_div((-cg),(a*sg))
    frac[0][2]=old_div((cg*ca-cb),(a*vol*sg))
    frac[1][0]=0.0
    frac[1][1]=1.0/(b*sg)
    frac[1][2]=old_div((cg*cb-ca),(b*vol*sg))
    frac[2][0]=0.0
    frac[2][1]=0.0
    frac[2][2]=old_div((sg),(c*vol))
    ...
    orth[1][0]=0.0
    orth[1][1]=b*sg
    orth[1][2]=old_div(c*(ca-cb*cg),sg)
    orth[2][0]=0.0
    orth[2][1]=0.0
    orth[2][2]=c*sb*sa
...
def position_cif_table(cif, table):
    ...
    for i in range(0, m):
        if table in cif[i].strip()[:ntab]:
            n1=i
            if i>0 and 'loop_' in cif[i-1] : n1=i-1
            for j in range(i, m):
                x=cif[j].lstrip()
                if ('#' in x[:1] or 'loop_' in x[:5] or
                    ('_' in  x[:1] and table not in x) or j==m-1) :
                    n2=j
                    break
            if n2>n1 : break
...
def get_software_version(logfile, progid, dic):
    ...
    if not check_file(10,logfile): return
    ...
    for x in fp:
        if 'MAPMAN' in progid.upper() and 'Created by MAPMAN V.' in x :
            ...
            prog='?'
            if len(linesplit)>0 : prog=linesplit[0].upper()
            ...
            if 'software' in dic and ss not in dic['software'] : dic['software'].append(ss)
            break
